...
# Create a new data frame called gapminder07 contaning only those rows in the gapminder data 
# where year is 2007

gapminder07<-gapminder[gapminder$year==2007,]
#OR gm2007<-subset(gapminder, subset=year==2007)
...
head(gapminder07[order(gapminder07$pop, decreasing=TRUE),],5)
...
# Use a t-test to evaluate the difference between 'gdpPercap' in "high" and "low" life expectancy 
# countries. Store the results as t1, and then print out t1.
...
# Save the gapminder07 data frame as a CSV file using write.csv() in the "data" subfolder within the 
# working directory
...
hist(gapminder07$lifeExp, 
     main = "Histogram of Life Expectancy across Countries in 2007",
     xlab = "Life Expecancy", ylab = "Frequency")
...
plot(lifeExp ~ gdpPercap, data=gapminder07,
     main = "Scatterplot of Life Expectancy and GDP Per Capita",
     xlab = "GDP per Capita", ylab = "Life Expecancy")
abline(h=mean(gapminder07$lifeExp), col="red")
